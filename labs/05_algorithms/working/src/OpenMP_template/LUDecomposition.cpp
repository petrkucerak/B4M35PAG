...
int main(int argc, char **argv)
{
  ...
  - You will have to iterate throw each row
     ...
     - Use equations from the slides to update rest of the row elements of
  matrix L
     - Use equations from the slides to update rest of the row elements of
  matrix U
...
#pragma omp for schedule(dynamic, chunk_size)
         for (int j = 0; j < size; ++j) {
            // For L matrix set for given row each element above the diagonal to
            // 0
            if (j < i) {
               l[j][i] == 0;
               continue;
            ...
            float reduction = a[j][i];
#pragma omp simd reduction(+ : reduction)
            for (auto k = 0; k < i; ++k)
               reduction = reduction - l[j][k] * u[k][i];
            l[j][i] = reduction;
         }
#pragma omp for schedule(dynamic, chunk_size)
         for (auto j = 0; j < size; ++j) {
            ...
            float reduction = a[i][j] / l[i][i];
#pragma omp simd reduction(+ : reduction)
            for (int k = 0; k < i; ++k)
               reduction = reduction - ((l[i][k] * u[k][j]) / l[i][i]);
            u[i][j] = reduction;
         }
      }
   }
